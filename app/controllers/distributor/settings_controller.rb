class Distributor::SettingsController < Distributor::BaseController
  ...
  def spend_limit_confirmation
    ...
    if count.positive?
      ...
      render text: "Updating the minimum balance will halt #{count} customers deliveries. #{count_emailed.to_s + ' customers with pending orders will be emailed that their account has been halted until payment is made. ' if count_emailed.positive? && send_halt_email && current_distributor.send_email?}Are you sure?"
    else
      render text: "safe"
    end
  end
